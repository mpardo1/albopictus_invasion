...
# Paths --------------------------------------------------------
path_in <- "data/" # Path to row data
path_out <- "data/output/" # Path to processed data
...
# Write csv
write.csv(dist_mat, paste0(path_out, "dist_mat_com_ESP.csv"))
...
# Load ids corresponding table for the different spanish boundaries
NATCODE_mitma_pa <-read.csv( paste0(path_out,"comarca_mitma_NATCODE_ESP.csv")) 
head(NATCODE_mitma_pa)

# Join comarca mitma and natcode
comarca_NATCODE_mitma <- comarca_NATCODE %>% left_join(unique(NATCODE_mitma_pa[,c("id_mitma", "NATCODE")]))
...
path <- paste0(path_in,"/boundaries/mitma_municip.gpkg")
...
# Load observations 
pa <- read.csv(paste0(path_out,"detection_albopictus.csv")) # Computed in 0_Weather_mob_process/01_process_aemet_cmip6.R
comarca_NATCODE_mitma_pa <- comarca_NATCODE_mitma %>% 
 left_join(pa)
...
comarcas_d <- read_sf(paste0(path_in,"ComarcasAgrarias.shp"))
...
# Save yearly detection data for Aedes albopictus for comarcas 
write.csv(comarca_pa,paste0(path_out,"pa_com.csv"))

# Compute flows between comarcas ---------------------------------------------
# Data from extract_mob_data.R
path <- paste0(path_in,"od_flows/mitma_municip/mean_daily_trips_apr_2023_nov_2023.csv")
flows_df <- setDT(read.csv(path))
flows_df <- flows_df[flows_df$id_origin != flows_df$id_destination,] # Remove flows with the same origin and destination
...
write.csv(flows_comarca_df, paste0(path_out,"flows_com_df.csv"))
...
write.csv(flows_mat,
          paste0(path_out,"od_flows/mitma_municip/flows_apr_2023_nov_2023_mitma_ESP_com_v2.csv"))
...
# Compute R_M by comarcas ----------------------------------------------- 
library(data.table)
source("funcR0.R") # Load code with function to compute RM
year = 2005
# Processed data from ERA5LAnd extracted from 0_Weather_mob_process/03_join_monthly_ERA5_data_to_year.R
Path <- paste0(path_out,"3_clim_df_",year,".Rds")
...
pop_df <- read.csv(paste0(path_in,"pobmun/pobmun",substr(as.character(year),3,4),".csv"))
...
# Join Spanish shapefile to compute area with number of inhabitants csv
...
# ggplot( esp_can %>% left_join(pop_df))+ geom_sf(aes(fill = log10(dens))) + scale_fill_viridis_c(option="D") # Test
...
for(i in c(2006:2023)){
  ...
  pop_df$dens <- pop_df$pob/pop_df$area
  pop_df <- pop_df[!is.na(pop_df$area),]
  # ggplot( esp_can %>% left_join(pop_df))+ geom_sf(aes(fill = dens)) + scale_fill_viridis_c(option="D") # Test
...
write.csv(df_clim_ESP,paste0(path_out,"3_rm_alb_ESP_com_0_2_v2.csv"))

# Average RM ---------------------------------------------
RM_ESP <- read.csv(paste0(path_out,"3_rm_alb_ESP_com_0_2.csv"))
RM_RM <- colSums(RM_ESP[, c(3:ncol(RM_ESP))]/nrow(RM_ESP))
tmin_ESP <- read.csv(paste0(path_out,"min_temp_ESP.csv"))
tmin_ESP_avg <- colSums(tmin_ESP[, c(3:ncol(tmin_ESP))]/nrow(tmin_ESP))
tmin_ESP <- apply(tmin_ESP[, c(3:ncol(tmin_ESP))], 2, min)

write.csv(RM_RM, paste0(path_out,"3_rm_alb_ESP_com_0_2.csv"))
