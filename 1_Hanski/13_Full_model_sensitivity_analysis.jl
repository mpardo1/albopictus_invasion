...
# To run the code be on the albopictus_invasion directory if not change path adding the path to albopictus_invasion directory in line 21
...
matrix_obs = matrix_obs[:,3:end] # Years 2005-2023 included
...
t_vect=1:tf
u0 = pop_init
p= [0.000571, 97.78894801161957, 5424.950376420903,
...
# Parameters bounds
low_bound = Float64[0,0,0,0,100,-50,-500]
...
p_rand = 0.05 # % of the coefficients randomly changed
n = round(Int, p_rand*length(matrix_obs))
inds = randperm(length(matrix_obs))[1:n]

# Loop through different initial parameters parallely --------------------------------------------
for i in 1:20
  init_param = [0.000571, 97.78894801161957, 5424.950376420903,
  6.97762397002697e-5, 51314.7750145224, -6.874687389443576, -80.94731156667044]
  ...
  try
    ...
    optsol = Optimization.solve(optprob,OptimizationOptimJL.LBFGS(), maxiters = 500)
