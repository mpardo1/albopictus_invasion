...
process_CERRA <- function(Path){
  clim_df <- rast(Path)
  clim_df <- tapp(clim_df, index = "days", fun = "mean") # Aggregate daily
  time_vec <- time(clim_df) # Extract time for after
  print(time_vec)[1]
  # plot(clim_df[[23]]) # plot to see the extension of the raster
  # Extract by mask
  clim_df <- terra::project(clim_df,muni_mitma, method = "average")
  ...
  clim_df <- terra::extract(clim_df,muni_mitma,
                            method = "bilinear", ID=TRUE)
  colnames(clim_df)[2:ncol(clim_df)] <- time_vec
  clim_df1 <- reshape2::melt(clim_df, id.vars = c("ID"))
  clim_df1$value <- clim_df1$value - 273.5 # Transform to kelvin
  clim_df1 <- clim_df1 %>% group_by(ID,variable) %>%
    summarise(tmean = mean(value, na.rm = TRUE),
              tmin = min(value, na.rm = TRUE),
              tmax = max(value, na.rm = TRUE)) 
  df_time <- data.frame(variable = unique(clim_df1$variable),
                        time = time_vec)
  clim_df1 <- clim_df1 %>% left_join(df_time) %>% 
    left_join(id_mitma_df)
  clim_df1 <- clim_df1[,c("id_mitma","time","tmean", "tmin", "tmax")]
  clim_df1$date <- as.Date(clim_df1$time)
  saveRDS(clim_df1,paste0(path_output,"era5_land_temp_mitma_",unique(month(clim_df1$date)),
