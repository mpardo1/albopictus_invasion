...
path_pc <- "~/albo_mobility/"
path_out <- "~/output_phase_space/"

# Load data
flows_mat <- read.csv(paste0(path_pc,"data/InputHanski/ESP/flows_apr_2023_nov_2023_mitma_ESP_com_v2.csv"))
eta <- as.matrix(flows_mat[,-1])
diag(eta) <- 0
dist_mat <- read.csv(paste0(path_pc,"data/InputHanski/ESP/dist_mat_com_ESP.csv"))
...
df_clim_ESP <-read.csv(paste0(path_pc,"data/InputHanski/ESP/rm_alb_ESP_com_future.csv"))
df_clim_ESP <- df_clim_ESP[year(df_clim_ESP$date) < year_f,]
mean_RM <- colSums(df_clim_ESP[,c(4:ncol(df_clim_ESP))])/nrow(df_clim_ESP)
max_RM <- apply(df_clim_ESP[,c(4:ncol(df_clim_ESP))], 2,max)
N = nrow(eta)
vec_RM <- matrix(0,N,N)
diag(vec_RM) <- mean_RM

# Save as a Rds to plot
saveRDS(mean_RM, "data/InputHanski/ESP/mean_RM_fut_ESP.Rds")

# Compute an average yearly minimum temp --------------------------------------------
df_tmin_ESP <-read.csv(paste0(path_pc,"data/InputHanski/ESP/min_temp_ESP_future.csv"))

# compute minimum year
df_tmin_ESP <- df_tmin_ESP[,c(3:ncol(df_tmin_ESP))] %>%  
...
# Save as a Rds to plot
saveRDS(min_tmin, "data/InputHanski/ESP/min_temp_yearly_mean_fut_ESP.Rds")
...
alp <- param[5]
# mat[1:5,1:5]
lambda_M_func(c1,0.2,c3,cd,400,e1,e2)
lambda_M_func(c1,0.0001,c3,cd,alp,e1,e2)
lambda_M_func(c1,c2,c3,cd,100,e1,e2)
lambda_M_func(c1,2.5,c3,cd,1000,e1,e2)
lambda_M_func(c1,0.01,c3,cd,200,e1,e2)
lambda_M_func(c1,c2,c3,cd,alp,e1,e2)
lambda_M_func(c1,c2,c3,cd,alp,e1,-5)
lambda_M_func(c1,c2,c3,cd,alp,e1,-7)
lambda_M_func(c1,0.1,c3,cd,alp,e1,-9)
lambda_M_func(c1,17,c3,cd,alp,e1,-9)
...
path_out <- "/home/marta/Documentos/PHD/2025/Colonization/output/output_phase_space/output_phase_space/"
saveRDS(rbind(grid_df, grid_df1), paste0(path_out,"phase_space_dist_c1_vs_ext_fut_v2_",Sys.Date(),".Rds"))
...
# Save data frame# Save data framelambda_M_func()
path_out <- "/home/marta/Documentos/PHD/2025/Colonization/output/output_phase_space/output_phase_space/"
saveRDS(rbind(grid_df, grid_df1), paste0(path_out,"phase_space_dist_cd_vs_ext_fut",Sys.Date(),".Rds"))
