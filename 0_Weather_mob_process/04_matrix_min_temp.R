...
# Download data from ERA5 Land albopictus_invasion/0_Weather_mob_process/01_download_ERA5LAnd.py
Path <- paste0(path_in,"temp/")
...
transform_minTemp <- function(i){
  ...
  rast_clim <- crop(rast_clim, ext(-10,5,35,45)) # Crop the raster for Peninsula iberica
  rast_clim <- tapp(rast_clim, index= "days", fun = "min") # Compute minimum by day
...
saveRDS(mat_mintemp,
        paste0(path_out, "min_temp_ESP",init_ind,"_",end_ind,".Rds"))
# mat_mintemp <- readRDS("~/albo_mobility/data/InputHanski/ESP/min_temp_ESP.Rds")

 # Join all files from clc
 list_files <- list.files(path_out, pattern = "min_temp")
...
# Future min temp ------------------------------------------------------------
min_temp <- paste0(path_out,"min_temp_Comarcas_2023_2033.Rds")
...
# Save as csv
write.csv(min_temp[,-ncol(min_temp)], paste0(path_out, "min_temp_ESP_future.csv"))
...
# Save as csv
write.csv(rM_fut[-1,],paste0(path_out, "rm_alb_ESP_com_future.csv"))
write.csv(min_temp,paste0(path_out, "min_temp_ESP_future.csv"))
